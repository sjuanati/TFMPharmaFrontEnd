...
import { Badge, Spinner, Header, Item, Text, Container, Toast, ListItem, Right, Body, Left, Icon, Input } from "native-base";
...
const getOrders = ( props ) => {
  ...
  const headerArray = [{ order_id:0, header: true }];
  ...
  const StatusOrder = ({status}) => {
    if(status === 0) {
      ...
      return (<Text style={{color: '#f0ad4e', fontSize: 10}}>PRECIO SOLICITADO</Text>)
    } else if(status === 2) {
      return (<Text style={{color: 'grey', fontSize: 10}}>PENDIENTE CONFIRMACIÓN</Text>)
    } else if(status === 3) {
      return (<Text style={{color: '#f0ad4e', fontSize: 10}}>EN PREPARACIÓN</Text>)
    } else if(status === 4) {
      return (<Text style={{color: 'grey', fontSize: 10}}>LISTO PARA RECOGER</Text>)
    } else if(status === 5) {
